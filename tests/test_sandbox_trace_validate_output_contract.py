...
def test_sandbox_trace_native_json_pass_contract() -> None:
...
def test_sandbox_trace_native_json_fail_contract() -> None:
...
def test_sandbox_trace_envelope_v2_pass_contract() -> None:
    ...
    assert data["meta"]["trace_id"] == "trace-sandbox-subprocess-0001"
    assert data["meta"]["request_id"] == "sandbox-request-success-subprocess-0001"
    assert data["meta"]["failure_code"] is None

    payload_result = next(result for result in data["payload"]["results"] if result["code"] == "sandbox_trace_checked")
    assert payload_result["validator"] == "sandbox-trace"
    assert payload_result["details"]["trace_id"] == "trace-sandbox-subprocess-0001"
    ...
    message = next(message for message in data["messages"] if message["code"] == "sandbox_trace_checked")
    assert message["severity"] == "info"
    assert message["status"] == "pass"
    assert message["path"] == VALID
    assert message["details"]["validator"] == "sandbox-trace"
    assert message["details"]["trace_id"] == "trace-sandbox-subprocess-0001"
...
def test_sandbox_trace_envelope_v2_fail_contract() -> None:
...
def test_sandbox_trace_shaped_invalid_schema_contract(tmp_path: Path) -> None:
...
def test_sandbox_trace_shaped_missing_schema_contract(tmp_path: Path) -> None:
    ...
    assert error["details"]["replay_execution"] is False
    assert error["details"]["mutation_performed"] is False
    assert not any(item["code"] == "sandbox_trace_checked" for item in result["results"])


def test_sandbox_trace_unrelated_json_not_misclassified_contract() -> None:
