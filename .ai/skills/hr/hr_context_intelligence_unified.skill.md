---
name: HR Context Intelligence Unified
type: skill
id: SKILL-HR-CONTEXT-INTEL-UNIFIED-v1
version: 2.2.0
updated: 2026-04-14
scope: hr
used_by: [hr]
...
Select the context intelligence mode by matching keywords in the input:

| Mode | Keywords |
|------|----------|
| CONTEXT_AWARENESS | structure, relationship, mapping, system, agent |
| SKILL_INVENTORY | inventory, catalog, list, available, skill |
| GAP_RECOGNITION | gap, missing, inconsistency, coverage, alignment |
...
**Permitted operations**: scan, analyze, identify, extract, observe, verify, catalog, classify, track, monitor

**Forbidden operations**: create, modify, delete, update, change, allocate, recommend, decide, execute, implement

Any write operation in read-only context intelligence must be rejected.
...
- Write operations: create, modify, delete, update, change, allocate
- Decision operations: recommend, decide, execute, implement, manage
- Authority operations: approve, reject, assign, deploy, configure

**Scope Validation**: Read-only scope must be validated. Reject if write operations are detected.
