...
## Multi-Tool Usage (Cursor, GitHub Copilot, Claude, Windsurf)

This `.ai/` folder is the single source of truth for system rules. All four tools consume it via symlinks or `mcp install` / `mcp sync`.

| Tool | Rule loading | Output path | SSoT |
|------|--------------|-------------|------|
| Cursor | symlink `.cursor/rules` ->`.ai/rules` or mcp install | `.cursor/rules` or `.cursorrules` | `.ai/rules/rules.md` |
| GitHub Copilot | symlink or mcp install | `.github/copilot-instructions.md` | `.ai/rules/rules.md` |
| Claude Code | symlink `CLAUDE.md` or mcp install | `CLAUDE.md` or `.claude/project.md` | `.ai/rules/rules.md` |
| Windsurf | symlink `.windsurf/rules` ->`.ai/rules` or mcp install | `.windsurf/rules` or `.windsurfrules` | `.ai/rules/rules.md` |

**Updating rules**: Edit only `.ai/rules/rules.md`. With symlinks, changes apply immediately. With mcp, run `mcp sync` (or `mcp install --all-tools`) to regenerate tool-specific files.
