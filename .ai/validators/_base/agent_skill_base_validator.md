...
### 4.4 Code Quality Standards (Universal)
| Metric | Target | Measurement |
|--------|--------|-------------|
| Hard coding detection | 95%+ | No absolute paths/hardcoded values |
| Configuration externalization | 95%+ | Environment variables/config files |
| Path management | 100% | Relative paths, cross-platform compatible |
| Technical debt impact | Measured | Quantified and prioritized |

### 4.5 Self-Validation Logic (Built-in)
```python
def universal_hard_coding_validation(code_content):
    """Built-in hard coding detection - no external dependencies"""
    violations = []
    
    # Absolute path patterns
    if re.search(r'[A-Za-z]:\\[^"\'\s]+', code_content):  # Windows
        violations.append("Windows absolute path detected")
    if re.search(r'/[^"\'\s]+', code_content):  # Unix
        violations.append("Unix absolute path detected")
    
    # Hardcoded URLs
    if re.search(r'https?://[^"\'\s]+', code_content):
        violations.append("Hardcoded URL detected")
    
    # Magic numbers/strings
    if re.search(r'timeout\s*=\s*\d{4,}', code_content):
        violations.append("Magic number detected")
