...
Sandbox trace rule promotion audit이 완료되었고 split promotion을 권장했다. Primary target은 `.ai/rules/operations/validation.rules.md`이며, secondary pointer는 `.ai/rules/operations/sync.rules.md`이다.

다음 안전한 방향은 sandbox trace validation behavior를 runtime governance rules에 승격하는 것이다. 이 승격은 정적 검증 boundary만 문서화하며 sandbox launcher, subprocess execution, provider execution, replay execution, generated content, snapshot update, sync apply/mutation을 승인하지 않는다.

Sandbox trace validation behavior가 runtime governance rules에 승격되었다. `.ai/rules/operations/validation.rules.md`는 command/output/static-only boundary를 담고, `.ai/rules/operations/sync.rules.md`는 future sandbox/provider safety pointer만 담는다.
...
Execution architecture approval plan과 risk audit이 완료되었다. 승인된 범위는 future execution prototype planning으로 넘어가기 위한 design-only boundary이며, sandbox launcher, subprocess execution, provider execution, replay execution, generated content, snapshot update, sync apply/mutation 구현은 계속 금지된다.
