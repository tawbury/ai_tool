...
다음 안전한 방향은 real preview provider implementation으로 바로 가기 전에 replay validation governance rule promotion 필요성을 검토하는 것이다.

권장 다음 작업:

1. Replay validation governance rule promotion 필요성 검토
2. Real provider execution 없이 replay comparison validator 설계 여부 결정
